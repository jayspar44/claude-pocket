...
module.exports = {
  apps: [
    ...
    // DEV app - port 4502, runs from separate folder, connects to DEV relay (4503)
    {
      name: 'claude-pocket-app-dev',
      cwd: '../claude-pocket-dev/app',
      script: 'node_modules/.bin/vite',
      args: 'preview --host 0.0.0.0 --port 4502',
      env: {
        NODE_ENV: 'production',
      },
      max_restarts: 10,
      min_uptime: '10s',
      restart_delay: 1000,
      error_file: '~/.pm2/logs/app-dev-error.log',
      out_file: '~/.pm2/logs/app-dev-out.log',
      merge_logs: true,
      log_date_format: 'YYYY-MM-DD HH:mm:ss',
    },
    // DEV relay - port 4503, runs from separate folder (separate Claude session)
    {
      name: 'claude-pocket-relay-dev',
      cwd: '../claude-pocket-dev/relay',
      script: 'src/index.js',
      env: {
        NODE_ENV: 'production',
        PORT: 4503,
