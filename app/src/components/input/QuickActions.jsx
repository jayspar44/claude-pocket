import { Check, X, StopCircle, CornerDownLeft, Slash, FolderOpen, Camera, RotateCcw, ChevronUp, ChevronDown, ChevronLeft, ChevronRight } from 'lucide-react';

const actions = [
  { id: 'yes', label: 'y', icon: Check, color: 'bg-green-600 hover:bg-green-700' },
  { id: 'no', label: 'n', icon: X, color: 'bg-red-600 hover:bg-red-700' },
  { id: 'interrupt', label: 'Ctrl+C', icon: StopCircle, color: 'bg-orange-600 hover:bg-orange-700' },
...
function QuickActions({ onAction, onOpenCommands, onOpenFiles, onOpenCamera, disabled = false }) {
  return (
    <div className="flex items-center gap-1 px-2 py-1.5 bg-gray-800 border-t border-gray-700 overflow-x-auto no-scrollbar">
      ...
      <div className="flex items-center gap-1">
        {actions.map((action) => (
          <button
            ...
            disabled={disabled}
            className={`flex items-center justify-center gap-1 px-2 py-1 text-xs font-medium text-white rounded-md ${action.color} disabled:opacity-50 disabled:cursor-not-allowed transition-colors whitespace-nowrap`}
      ...
      <div className="flex items-center gap-0.5">
        {arrowActions.map((action) => (
          <button
            ...
            onClick={() => onAction(action.id)}
            disabled={disabled}
            className={`flex items-center justify-center p-1 text-white rounded ${action.color} disabled:opacity-50 disabled:cursor-not-allowed transition-colors`}
      ...
      <button
        ...
        disabled={disabled}
        className="flex items-center justify-center gap-1 px-2 py-1 text-xs font-medium text-white rounded-md bg-gray-600 hover:bg-gray-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors whitespace-nowrap shrink-0"
        aria-label="Clear"
      >
